...
def configure_sparse(model, args):
    ...
    if args.sparse:
        ...
        model.config.sparse_config = sparse_config

    else:
        model.config.sparse_config = None
...
def collate_fn(batches, tokenizer):
    ...
    return images, texts, bboxes, hws
...
def evaluate_chat_model():
    ...
    for ds_name in args.datasets:
        ...
        outputs = []
        for _, (img, questions, bboxes, hws) in enumerate(tqdm(dataloader)):
            ...
            generated_text = (
                tokenizer.decode(generated_id[0])
                .replace("<s>", "")
                .replace("</s>", "")
                .strip()
            )
            ...
            for bbox, hw, answer in zip(bboxes, hws, answer):
                outputs.append({
                    'answer': answer,
                    'gt_bbox': bbox,
                    'hw': hw,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--a', type=int, default=4, help='window size')
    ...
    prompt = 'Please provide the bounding box coordinate of the region this sentence describes: <ref>{}</ref> (Coordinate system: x-axis is horizontal, y-axis is vertical, with the top-left corner as the origin [0.0, 0.0] and the bottom-right corner as [1.0, 1.0])'
